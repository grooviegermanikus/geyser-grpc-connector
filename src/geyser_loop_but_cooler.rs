...
use std::str::FromStr;
use yellowstone_grpc_proto::geyser::{SubscribeUpdateSlot, SlotStatus as ySS, SubscribeUpdate};
use anyhow::anyhow;
    ...
    pub fn consume(&mut self, update: &SubscribeUpdate) -> Effect {
        ...
        match update.update_oneof.as_ref() {
            Some(UpdateOneof::Slot(msg)) => {
                // this is important
                let commitment_status = map_slot_status(msg).unwrap();
                if self.skip_til_this_slot == None && commitment_status == SlotStatus::Processed {
                    self.skip_til_this_slot = Some(msg.slot);
                    return Effect::Noop;
                }
                if commitment_status != SlotStatus::Confirmed {
                    return Effect::Noop;
                }
